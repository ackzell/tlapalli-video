...
function pointsToPath(points: Point[]): string {
  const parts = points.map(([x, y], index) => `${index === 0 ? 'M' : 'L'} ${x.toFixed(3)} ${y.toFixed(3)}`);
  return `${parts.join(' ')} Z`;
}
...
function normalizePath(raw: RawGemPath): GemPathData {
  ...
  const shift = (input: string) =>
    parsePoints(input).map(([x, y]) => [x - cx, y - cy] as Point);
...
const GEM_PATHS = Object.fromEntries(
  (Object.entries(RAW_GEM_PATHS) as [GemName, RawGemPath][]).map(([name, raw]) => [name, normalizePath(raw)]),
